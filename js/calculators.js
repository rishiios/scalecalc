export const calculators = {
  ...
  compound: {
    ...
    calculate: (inputs) => {
      ...
      const chartPoints = [{ x: 0, y: initialDeposit }];
  ...
  std_deviation: {
    ...
    calculate: (inputs) => {
      ...
      const numbers = str.split(',').map(s => parseFloat(s.trim())).filter(n => !isNaN(n));
  ...
  subnet: {
    ...
    calculate: (inputs) => {
      ...
      const netOctets = ipParts.map((b, idx) => b & maskOctets[idx]);
